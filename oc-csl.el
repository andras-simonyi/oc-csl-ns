...
(defun org-cite-csl--no-citelinks-p (info)
  ...
  (or (not org-cite-csl-link-cites)
      ...
      ;; The used style is not not a note style and no bibliography is
      ;; exported.
      (and (not (org-element-map (plist-get info :parse-tree) 'keyword
                  (lambda (k)
                    (equal "PRINT_BIBLIOGRAPHY" (org-element-property :key k)))
                  info t))
           (not (citeproc-style-cite-note
                 (citeproc-proc-style (plist-get info :cite-citeproc-processor)))))))
